...
bool HTMLTokenizer::handleState(char cc)
{
    ...
    case State::CDATASectionRightSquareBracketState:
        return handleCDATASectionRightSquareBracketState(cc);
    case State::CDATASectionDoubleRightSquareBracketState:
...
{
    if(cc == ']')
        return advanceTo(State::CDATASectionRightSquareBracketState);
    if(cc == 0)
        return switchTo(State::Data);
...
{
    if(cc == ']')
        return advanceTo(State::CDATASectionDoubleRightSquareBracketState);
...
bool HTMLTokenizer::consumeCharacterReference(std::string& output, bool inAttributeValue)
{
    ...
    ParserString input(m_input.substring());
    HTMLEntityParser entityParser(output, input, inAttributeValue);
    if(!entityParser.parse())
        return false;

    m_input += input.offset();
    return true;
}
